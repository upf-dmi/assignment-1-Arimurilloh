...
heart_data <- read.csv(file.path("data", "heart_disease_dataset.csv"), sep = " ", header = TRUE, na.strings = c("?"))
...
grid.arrange(grobs = plot_list, ncol = 3)
```
The prior plots allows to visualize each variable individually. It would also be interesting to see how variables interact each other, something that we can observe with scatterplots, for instance, by plotting one variable in front of another. 

Following we plot the age vs. the other continuous variables. It is difficult to see clear clusters of patients groups, but some patterns can be observed. For instace, we see a slight tendence in which the maximum heart beat rate is reduced by age in group 0, but this tendence is not so clear in the other groups.
...
![Original Figure 1](https://www.frontiersin.org/files/Articles/987182/fmed-09-987182-HTML/image_m/fmed-09-987182-g001.jpg)

```{r}
data <- read_excel(file.path("data", "data_covid19.XLSX"), col_names = TRUE)
...
As can be seen, the figure to reproduce is formed by four subplots on a 2x2 grid. We will proceed to build each of the subplots of the figure. 

For the first one (A), we have an histogram of the age of the patients, showing the absolute frequency on the dataset. We can obtain this histogram simply with ```ggplot2``` package.
...
![Original Figure 2 from the article.](https://www.frontiersin.org/files/Articles/987182/fmed-09-987182-HTML/image_m/fmed-09-987182-g002.jpg)
...
file_path <- file.path("data", "data2_covid19.XLSX")

# Check if the file exists before reading it
if (file.exists(file_path)) {
  
  # Read the Excel file, skipping the first row
  df <- read_excel(file_path, col_names = TRUE, na = c("NI", "ND"), skip = 1)
  
  # Clean column names to ensure they are valid in R
  df <- clean_names(df)  
  
  # Ensure at least two columns exist before renaming
  if (ncol(df) >= 2) {
    colnames(df)[1:2] <- c("ID", "Day")
  } else {
    stop("The dataset has fewer than 2 columns. Please check the data file.")
  }
  
  # Extract only numbers from Patient_ID, handling potential missing values
  df <- df %>%
    mutate(ID = str_extract(ID, "\\d+$")) %>%
    fill(ID, .direction = "down")  # Fill missing Patient_ID values downward
...
```{r}
# Select only cytokine-related columns
cytokine_cols <- colnames(df)[3:14]  

# Convert selected columns to numeric
df[cytokine_cols] <- lapply(df[cytokine_cols], as.numeric) 

# Compute median per patient across time points (corrected syntax for `across()`)
df_median <- df %>%
  group_by(ID) %>%
  summarise(across(all_of(cytokine_cols), \(x) median(x, na.rm = TRUE)))  # Corrected function

# Build clinical info dataframe
...
# Merge cytokine data with clinical info
df_median <- inner_join(df_ex1, df_median, by = "ID")
df_median <- df_median[order(df_median$Group),]  # Order by groups for proper annotation

# Select only cytokine values (scaled) and remove NaNs (assign 0)
cytokine_data <- t(scale(df_median[,5:16])) 
cytokine_data[is.na(cytokine_data)] <- 0  # Replace NA values with 0
```
